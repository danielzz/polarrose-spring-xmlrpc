...
package com.polarrose.xmlrpc.handlers;

import com.polarrose.xmlrpc.XmlRpcArray;
import com.polarrose.xmlrpc.XmlRpcException;
import com.polarrose.xmlrpc.XmlRpcInvocationHandler;
import com.polarrose.xmlrpc.XmlRpcMessages;
...
public class ReflectiveInvocationHandler implements XmlRpcInvocationHandler
{
            ...
            if ( method.getName().equals( methodName ) )
            {
                ...
                if ( parameterTypes.length == argClasses.length )
                {
                    for (int j = 0; j < parameterTypes.length; ++j)
                    {
                        ...
                        else if (!parameterTypes[j].isAssignableFrom(argClasses[j]) && !isValueAssignableToType(argValues[j], parameterTypes[j]))
                        {
                            continue outer;
                        }
                    }
                    ...
                    for (int j = 0; j < parameterTypes.length; ++j) {
                        if (requiresConversion(argValues[j], parameterTypes[j])) {
                            Object o = convertObject(argValues[j], parameterTypes[j]);
                            if (o == null) {
                                continue outer;
                            }
                            argValues[j] = o;
    ...
    // TODO The following code is extremely flakey - Needs a complete rewrite.

    private Object convertObject(Object argValue, Class parameterType)
    {
        if (argValue.getClass().equals(XmlRpcArray.class))
        {
            XmlRpcArray array = (XmlRpcArray) argValue;

            if (parameterType.isArray())
            {
                Class arrayElementClass = parameterType.getComponentType();

                Object a = Array.newInstance(arrayElementClass, array.size());
    ...
    private boolean requiresConversion(Object argValue, Class parameterType)
    {
        return isValueAssignableToType(argValue, parameterType);
    }

    private boolean isValueAssignableToType(Object argValue, Class parameterType)
    {
        if (argValue.getClass().equals(XmlRpcArray.class))
        {
            XmlRpcArray array = (XmlRpcArray) argValue;
            ...
            if (parameterType.isArray())
            {
                ...
                if (arrayElementClass == int.class) {
                    for (int i = 0; i < array.size(); i++) {
                        if (array.get(i).getClass() != Integer.class) {
                            return false;
                        }
                    }
                    return true;
                    ...
                    for (int i = 0; i < array.size(); i++) {
                        Object o = array.get(i);
                        if (!arrayElementClass.isAssignableFrom(o.getClass())) {
                        //if (!o.getClass().equals(arrayElementClass)) {
                            return false;
                        }
                    }
                    return true;
